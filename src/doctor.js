...
function checkCommand(command, description) {
  try {
    ...
    return { passed: false, message: description, details: `Command not found: ${command}` };
  }
}

function checkDirectoryNotEmpty(dir, description) {
  if (!fs.existsSync(dir)) {
    return { passed: false, message: description, details: `Directory missing: ${dir}` };
  }
...
function checkClaudeMdSections(claudeMdPath) {
  if (!fs.existsSync(claudeMdPath)) {
    return { passed: false, message: 'CLAUDE.md has required sections', details: 'File missing' };
  }
...
function checkFarmhouseFormat(farmhousePath) {
  if (!fs.existsSync(farmhousePath)) {
    return { passed: false, message: 'FARMHOUSE.md follows format', details: 'File missing' };
  }
...
function checkGitignore(cwd) {
  ...
  if (!fs.existsSync(gitignorePath)) {
    return { passed: false, message: '.gitignore includes local settings', details: 'File missing' };
  }
...
export async function doctor() {
  ...
  checks[0].items.push(checkExists(path.join(cwd, '.claude', 'settings.json'), 'settings.json exists'));
  checks[0].items.push(checkClaudeMdSections(path.join(cwd, 'CLAUDE.md')));

  checks.push({ category: 'Agents & Commands', items: [] });
  checks[1].items.push(checkDirectoryNotEmpty(path.join(cwd, '.claude', 'agents'), 'Has agents defined'));
  checks[1].items.push(checkDirectoryNotEmpty(path.join(cwd, '.claude', 'commands'), 'Has commands defined'));

  checks.push({ category: 'Audit System', items: [] });
  checks[2].items.push(checkExists(path.join(cwd, '_AUDIT'), '_AUDIT/ directory exists'));
  checks[2].items.push(checkExists(path.join(cwd, '_AUDIT', 'FARMHOUSE.md'), 'FARMHOUSE.md exists'));
  ...
  checks[3].items.push(checkExists(path.join(cwd, 'justfile'), 'justfile exists'));
  checks[3].items.push(checkCommand('just', 'just command available'));

  checks.push({ category: 'Issue Tracking', items: [] });
  checks[4].items.push(checkExists(path.join(cwd, '.beads'), '.beads/ directory exists'));
  ...
  const health = totalFailed === 0
    ? (totalWarnings === 0 ? 'Excellent' : 'Good')
    : (totalFailed <= 2 ? 'Needs Attention' : 'Critical');

  const healthColor = health === 'Excellent' ? chalk.green
    : health === 'Good' ? chalk.cyan
    : health === 'Needs Attention' ? chalk.yellow
    : chalk.red;
